...
* **Look at every file before it is listed for attachment.** The rules above cover what the comment *says*; this one covers what the evidence *shows*, which nobody has read yet. A real back office puts e-mail addresses in order pages and API client secrets in success banners, and a `transcript.txt` holds whole response bodies, because for a command line or an endpoint the output **is** the measurement and truncating it would destroy the evidence. So open every screenshot, watch the video, and read the transcript, before naming any of them in `index.md`. If a run needs a shop carrying real customer data, that is the wrong shop for this skill.
* **The video is the widest exposure of the three, and the only one that cannot be edited.** It records the whole pass, not the moments someone chose: every page visited, every banner, and the login form with the e-mail typed into it. A screenshot showing something that must not ship can be retaken with the field out of frame, or covered with a **solid box** — never a blur, which over a short string in a known font is partly recoverable, and would be recovered from a public pull request. A video offers no such repair: either it is clean and it ships, or it stays out and the paired screenshots carry the proof, with the comment saying the recording is available on request. Deciding that at the end costs a re-run, so it is worth knowing before the `after` pass which surfaces it will cross.
